...
.page-title { font-family: 'Syne', sans-serif; font-size: 2.2rem; font-weight: 800; color: #f0f4ff; letter-spacing: -0.02em; margin-bottom: 0.2rem; }
.page-sub { font-family: 'DM Mono', monospace; font-size: 0.78rem; color: rgba(150,180,220,0.5); margin-bottom: 1.8rem; letter-spacing: 0.05em; }
.section-label { font-family: 'DM Mono', monospace; font-size: 0.65rem; letter-spacing: 0.2em; text-transform: uppercase; color: rgba(150,180,220,0.4); margin-bottom: 0.7rem; margin-top: 1.5rem; }
.stat-card { background: #0d1628; border: 1px solid rgba(255,255,255,0.07); border-radius: 12px; padding: 1.2rem 1.5rem; position: relative; overflow: hidden; }
.stat-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px; background: linear-gradient(90deg, #00c882, transparent); }
.stat-label { font-size: 0.65rem; letter-spacing: 0.15em; text-transform: uppercase; color: rgba(150,180,220,0.5); margin-bottom: 0.4rem; }
.stat-value { font-family: 'Syne', sans-serif; font-size: 1.8rem; font-weight: 700; color: #f0f4ff; line-height: 1; margin-bottom: 0.2rem; }
.stat-sub { font-size: 0.68rem; color: #00c882; }
.sector-pill { display: inline-block; background: rgba(0,200,130,0.08); border: 1px solid rgba(0,200,130,0.2); border-radius: 999px; padding: 0.3rem 0.9rem; font-size: 0.72rem; color: rgba(0,200,130,0.85); margin-right: 0.4rem; margin-bottom: 0.4rem; }
.qualify-badge { background: rgba(0,200,130,0.12); border: 1px solid rgba(0,200,130,0.3); border-radius: 8px; padding: 0.8rem 1.2rem; margin-bottom: 0.5rem; display: flex; align-items: center; justify-content: space-between; }
.qualify-name { font-family: 'Syne', sans-serif; font-size: 0.9rem; font-weight: 700; color: #e8f0ff; }
.qualify-sym { font-size: 0.7rem; color: rgba(150,180,220,0.5); margin-top: 2px; }
.qualify-score { font-family: 'Syne', sans-serif; font-size: 1.1rem; font-weight: 700; color: #00c882; }
div[data-testid='stButton'] > button { background: transparent; border: 1px solid rgba(0,200,130,0.25); border-radius: 8px; color: #00c882; font-family: 'DM Mono', monospace; font-size: 0.7rem; letter-spacing: 0.06em; padding: 0.3rem 0.8rem; transition: all 0.2s; width: 100%; }
div[data-testid='stButton'] > button:hover { background: rgba(0,200,130,0.1); border-color: rgba(0,200,130,0.6); }
</style>
'''
st.markdown(CSS, unsafe_allow_html=True)

st.markdown('<div class="page-title">Sector and Industry Analysis</div>', unsafe_allow_html=True)
...
for col in ("PE Ratio", "EPS", "ROE", "ProfitMargin", "DebtToEquity", "MarketCap"):
...
sort_col_map = {"Market Cap": "MarketCap", "EPS": "EPS", "ROE": "ROE", "PE Ratio": "PE Ratio", "Profit Margin": "ProfitMargin"}
sort_col = sort_col_map[rank_by]
if sort_col in scoped_df.columns:
    scoped_df = scoped_df.sort_values(by=sort_col, ascending=(rank_by == "PE Ratio"), na_position="last")
sel_df = scoped_df if show_all else scoped_df.head(10)

def _score_hi(v, a):
    if v is None or a is None or pd.isna(v) or pd.isna(a): return "[?]"
    return "[OK]" if v >= a else "[~]" if v >= a * 0.8 else "[X]"
def _score_lo(v, a):
    if v is None or a is None or pd.isna(v) or pd.isna(a): return "[?]"
    return "[OK]" if v <= a else "[~]" if v <= a * 1.1 else "[X]"
def _score_de(v, a):
    if v is None or a is None or pd.isna(v) or pd.isna(a): return "[?]"
    return "[OK]" if v <= a else "[~]" if v <= 1.5 else "[X]"
def fmt_cap(val):
...
for _, row in sel_df.iterrows():
    ...
    green_count = sum(v == "[OK]" for v in icons.values())
    ...
    if green_count >= interp_cutoff:
        qualified.append(dict(r, _sym=sym))
...
if not chart_df.empty:
    ...
    fig_cap = go.Figure(go.Bar(
        ...
        orientation="h",
        marker=dict(color=chart_df["MarketCap"], colorscale=[[0, "#0d3d2a"], [1, "#00c882"]], showscale=False),
    ...
    fig_cap.update_layout(
        ...
        xaxis=dict(showgrid=False, title="Market Cap (Rs.B)", color="rgba(150,180,220,0.5)"),
        yaxis=dict(showgrid=False, color="rgba(180,200,230,0.8)"),
        margin=dict(l=10, r=20, t=10, b=30), height=320,
    )
...
if len(scatter_df) >= 3:
    ...
    fig_sc = go.Figure(go.Scatter(
        ...
        marker=dict(size=10, color=scatter_df["ROE"], colorscale=[[0, "#0d3d2a"], [1, "#00c882"]], showscale=False, line=dict(width=1, color="rgba(0,200,130,0.3)")),
        hovertemplate="<b>%{text}</b><br>PE: %{x:.1f}<br>ROE: %{y:.1f}%<extra></extra>",
    ))
    if avg_pe is not None:
        fig_sc.add_vline(x=avg_pe, line_dash="dot", line_color="rgba(0,200,130,0.3)", annotation_text="Avg PE", annotation_font_color="rgba(0,200,130,0.5)")
    if avg_roe_pct is not None:
        fig_sc.add_hline(y=avg_roe_pct, line_dash="dot", line_color="rgba(0,200,130,0.3)", annotation_text="Avg ROE", annotation_font_color="rgba(0,200,130,0.5)")
...
st.caption("All " + str(len(scoped_df)) + " companies" if show_all else "Top 10 by " + rank_by)
...
st.dataframe(display_df, use_container_width=True, column_config={
    "Score": st.column_config.ProgressColumn("Score", min_value=0, max_value=5, format="%d / 5"),
...
if qualified:
    ...
    st.info("No company meets " + interp_threshold + " green signals. Try lowering the threshold.")

st.markdown("<hr style='border:none;border-top:1px solid rgba(255,255,255,0.06);margin:2rem 0 1rem;'>", unsafe_allow_html=True)
st.markdown("<div style='font-family:monospace;font-size:0.65rem;color:rgba(150,180,220,0.3);'>Fundamentals sourced from Yahoo Finance - Industry averages from cached DB</div>", unsafe_allow_html=True)
