...
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Syne:wght@400;600;700;800&family=DM+Mon
...
industries = sorted(merged_df[merged_df["Big Sectors"] == sec_sel]["Industry"].dropna().uniqu
ind_sel = st.sidebar.selectbox("Industry", industries)
rank_by = st.sidebar.selectbox("Rank companies by", ["Market Cap", "EPS", "ROE", "PE Ratio"
...
green_count = sum(v == " score_str " for v in icons.values())
= " | ".join(f"{k} {v}" for k, v in icons.items())
r = {
"Symbol": sym,
"Company": name_lookup.get(sym, ""),
"PE": round(pe_val, 2) if pe_val is not None and pd.notna(pe_val) else No
"EPS": round(eps_val, 2) if eps_val is not None and pd.notna(eps_val) else No
"ROE %": round(roe_val, 2) if roe_val is not None and pd.notna(roe_val) else No
"Margin %": round(pm_val, 2) if pm_val is not None and pd.notna(pm_val) else No
"D/E": round(de_val, 2) if de_val is not None and pd.notna(de_val) else No
...
st.markdown('<div class="section-label">// market cap distribution — top 10</div>', unsafe_al
...
if len(scatter_df) >= 3:
st.markdown('<div class="section-label">// ROE vs PE — value vs quality</div>', unsafe_al
...
font=dict(family="DM Mono", color="rgba(180,200,230,0.7)", size=11),
xaxis=dict(showgrid=False, title="PE Ratio", color="rgba(150,180,220,0.5)", zeroline=
yaxis=dict(showgrid=False, title="ROE %", color="rgba(150,180,220,0.5)", zeroline=
margin=dict(l=10, r=20, t=10, b=30),
height=320,
)
...
st.info(f"No company meets {interp_threshold} green signals in this industry. Try lowerin
# ── Footer ────────────────────────────────────────────────────────────────────
st.markdown("<hr style='border:none;border-top:1px solid rgba(255,255,255,0.06);margin:2rem 0
st.markdown("""
<div style="font-family:'DM Mono',monospace;font-size:0.65rem;color:rgba(150,180,220,0.3)
Fundamentals sourced from Yahoo Finance via yfinance · Industry averages from cached
</div>
""", unsafe_allow_html=True)
