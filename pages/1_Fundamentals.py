...
# ─────────────────────────────────────────────────────────────────────────
# DEMERGER HISTORY (MOVED BEFORE CORPORATE ACTIONS)
# ─────────────────────────────────────────────────────────────────────────
...
demerger_sym = chosen_sym.upper() if chosen_sym else None
if demerger_sym in demerger_data or chosen_sym in demerger_data:
    key = demerger_sym if demerger_sym in demerger_data else chosen_sym
    
    st.markdown('<div class="section-label">// demerger history</div>', unsafe_allow_html=True)
    
    for demerger in demerger_data[key]:
...
if current_industry:
    st.markdown(f"**🏭 Industry:** {current_industry} | **Sector:** {current_sector}")
    ...
    if not industry_peers.empty:
        ...
        st.markdown(f"**Competitors in this industry ({len(industry_peers)} companies):**")
        
        # Show first 6 competitors as clickable buttons
        cols = st.columns(6)
        for idx, (_, peer) in enumerate(industry_peers.head(6).iterrows()):
            col = cols[idx % 6]
...
if st.session_state.get("comparison_stocks"):
    ...
    with st.spinner("Loading comparison data..."):
            ...
            try:
                if sym == chosen_sym:
                    ...
                    if comp_db is not None and not comp_db.empty:
                        ...
                        if comp_data.get("ROE") is None:
                            try:
                                if "ROE" in db_row.index and pd.notna(db_row["ROE"]):
                                    comp_data["ROE"] = float(db_row["ROE"])
                            except:
                                pass
                        
                        if comp_data.get("PE Ratio") is None:
                            try:
                                if "PE Ratio" in db_row.index and pd.notna(db_row["PE Ratio"]):
                                    comp_data["PE Ratio"] = float(db_row["PE Ratio"])
                            except:
                                pass
                        
                        if comp_data.get("Debt to Equity") is None:
                            try:
                                # Try multiple column names
                                if "DebtToEquity" in db_row.index and pd.notna(db_row["DebtToEquity"]):
                                    comp_data["Debt to Equity"] = float(db_row["DebtToEquity"])
                                elif "Debt to Equity" in db_row.index and pd.notna(db_row["Debt to Equity"]):
                                    comp_data["Debt to Equity"] = float(db_row["Debt to Equity"])
                            except:
                                pass
                        
                        if comp_data.get("Profit Margin") is None:
                            try:
                                if "ProfitMargin" in db_row.index and pd.notna(db_row["ProfitMargin"]):
                                    comp_data["Profit Margin"] = float(db_row["ProfitMargin"])
                                elif "Profit Margin" in db_row.index and pd.notna(db_row["Profit Margin"]):
                                    comp_data["Profit Margin"] = float(db_row["Profit Margin"])
                            except:
                                pass
                        
                        if comp_data.get("EPS") is None:
                            try:
                                if "EPS" in db_row.index and pd.notna(db_row["EPS"]):
        ...
        # ─────────────────────────────────────────────────────────────────────────
        # SIDE-BY-SIDE PRICE CHARTS WITH INTERVAL SELECTOR
        # ─────────────────────────────────────────────────────────────────────────
        
        st.markdown('<div class="section-label">// price charts comparison</div>', unsafe_allow_html=True)
        ...
        try:
            ...
            if overlay_data:
                ...
                colors = ["#00c882", "#6ec6ff", "#f5a623", "#ff4d6a", "#ccff90", "#80d8ff", "#ffd180", "#ff80ab"]
                
                for idx, (sym, prices) in enumerate(overlay_data.items()):
                ...
                fig_overlay.update_layout(
                    title="<b>Performance Comparison</b> (Base = 100)",
                    paper_bgcolor="#080d1a",
                    plot_bgcolor="#080d1a",
